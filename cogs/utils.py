...
# Adventure Game IDs are now loaded from the bot_config file
...
# Channel IDs (now loaded from bot_config.json, with TEST_CHANNEL_ID as default)
...
# Role IDs and other dynamic constants are now loaded from the config
...
async def channel_id_name_autocomplete(interaction: discord.Interaction, current: str):
    ...
    choices = []
    # Only show keys that end in "channel_id" for clarity
...
async def role_id_name_autocomplete(interaction: discord.Interaction, current: str):
    ...
    for key in role_ids_dict:
        # Check if the key value is an integer before adding to choices
...
def load_adventure_channel_id() -> Optional[int]:
    """Loads the designated adventure channel ID from bot_config.json."""
    return bot_config.get("ADVENTURE_MAIN_CHANNEL_ID")

def save_adventure_channel_id(channel_id: int):
    """Saves the designated adventure channel ID to bot_config.json."""
    update_dynamic_config("ADVENTURE_MAIN_CHANNEL_ID", channel_id)

def save_user_roles(user_id: int, role_ids: list[int]):
    """Saves a user's previous roles to a file."""
    data = load_data(USER_ROLES_FILE, {})
    data[str(user_id)] = role_ids
    save_data(data, USER_ROLES_FILE)

def load_user_roles(user_id: int) -> list[int]:
    """Loads a user's previous roles from a file."""
    data = load_data(USER_ROLES_FILE, {})
...
async def generate_duel_image(winner_avatar_url: str, loser_avatar_url: str) -> discord.File:
    ...
    try:
        ...
        # Apply grayscale to the loser's avatar to show they are defeated.
        ...
        # Paste avatars with a slight angle to give a sense of dynamic action
        winner_avatar_rotated = winner_avatar.rotate(5, expand=True)
        loser_avatar_rotated = loser_avatar.rotate(-5, expand=True)

        # ADJUSTED COORDINATES to move avatars closer together
        ...
        # Paste the clash overlay in the center
...
async def generate_solo_death_image(avatar_url: str) -> discord.File:
    ...
    try:
        ...
        # Apply grayscale to the avatar
        avatar_img = ImageOps.grayscale(avatar_img)
        avatar_img = avatar_img.convert("RGBA")

        # Create a red tint overlay for a more dramatic death effect
        ...
        # Paste the avatar and skull in the center
...
async def generate_win_image(winner_avatar_url: str) -> discord.File:
    ...
    try:
        ...
        # Position the medal overlay on top of the avatar
